...
def soft_bellman_ford_fw(graph: nx.DiGraph, source_node, costs: dict,
                       max_iterations: int = DEFAULT_MAX_ITER,
                       tolerance: float = DEFAULT_TOLERANCE) -> dict:
    """
    Computes the soft value function V_soft = -log Z using soft Bellman-Ford in forward direction.
    V_soft(i) = -log [ sum_{j in predecessors(i)} exp(-cost(j, i) - V_soft(j)) ]
    Handles graphs with cycles. Converges if costs guarantee paths from source are finite.
    """
    nodes = list(graph.nodes())
    V_soft = {node: np.inf for node in nodes}
    
    if source_node not in V_soft:
        raise ValueError(f"Source node {source_node} not in graph.")
        
    V_soft[source_node] = 0.0

    for iteration in range(max_iterations):
        V_soft_old = V_soft.copy()
        max_delta = 0.0

        # Iterate in topological order if DAG, otherwise iterate until convergence
        # For general graphs, simple iteration works but might be slower
        for node in nodes:
            if node == source_node:
                continue
            ...
            if not predecessors:
                # Node has no predecessors, its value remains inf unless it's the source
                continue

            # Calculate logsumexp term: logsumexp([-cost(j, node) - V_soft_old[j]])
            terms = []
            for j in predecessors:
                edge = (j, node)
                if edge in costs and np.isfinite(V_soft_old[j]):
                   terms.append(-costs[edge] - V_soft_old[j])

            if not terms:
                 # No path from source through predecessors with finite cost/value
                 new_value = np.inf
            else:
                 new_value = -logsumexp(terms)
        ...
        # Check for convergence
        if max_delta < tolerance:
            # print(f"Forward Soft Bellman-Ford converged in {iteration + 1} iterations.")
            return V_soft

    print(f"Warning: Forward Soft Bellman-Ford did not converge within {max_iterations} iterations. Max delta: {max_delta}")
    return V_soft
