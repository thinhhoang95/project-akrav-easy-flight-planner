...
def find_nearest_airport_to_target_distance(waypoint_lat, waypoint_lon, airports_df, target_distance=3.5):
    """
    ...
    # Calculate distances from waypoint to all airports
    distances = []
    for _, airport in airports_df.iterrows():
        dist = haversine_distance(waypoint_lat, waypoint_lon, 
    ...
    for idx, row in tqdm(routes_df.iterrows(), total=len(routes_df)):
        ...
        if len(first_waypoint) == 4:
            ...
            if first_lat is not None and first_lon is not None:
                ...
                airport_icao, distance, diff = find_nearest_airport_to_target_distance(
...
def main():
    ...
    print(f"Loaded {len(routes_df)} flight routes")
