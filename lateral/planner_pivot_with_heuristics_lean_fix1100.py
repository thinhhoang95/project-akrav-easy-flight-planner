...
def find_admissible_pivot_nodes_with_heuristics(G, node_from, nodes_to_exclude, max_depth=8, 
    ...
    """
    Finds admissible pivot nodes using a layered exploration combined with heuristic prioritization.
    
    This function performs a modified BFS expansion that prioritizes nodes within each depth level
    according to a heuristic function. It explores the graph from a starting node up to a maximum 
    depth, tracking both the depth (number of hops) and accumulated distance to each visited node.
    
    A node is admitted if (1) it can be found within the minimum number of hops, and for the same
    depth, the accumulated distance is minimal.
    
    Parameters:
        G (NetworkX graph): Graph with nodes having 'lat' and 'lon' attributes.
        node_from (str): Starting node for the exploration.
        nodes_to_exclude (set): Set of nodes to avoid during expansion.
        max_depth (int, optional): Maximum exploration depth. Defaults to 8.
        prevent_backtracking (bool, optional): Whether to prevent movements that go back in the
                                             opposite direction. Defaults to False.
        origin (str, optional): Origin waypoint for directional checks. Required if 
                               prevent_backtracking is True.
        dest (str, optional): Destination waypoint for directional checks and default heuristic.
                             Required if prevent_backtracking is True.
        direction (str, optional): Direction of exploration - 'forward' uses successors, 
                                  'backward' uses predecessors. Defaults to 'forward'.
        branching_factor (int, optional): Maximum number of nodes to explore at each depth layer.
                                        If None, all nodes are explored. Defaults to None.
        heuristic (callable, optional): Function that takes (node, current_distance) and returns
                                      a score (lower is better). If None and dest is provided,
                                      a default A*-like heuristic is used. Defaults to None.
    
    Returns:
        tuple: A tuple containing:
            - admissible_nodes (list): List of nodes that were admissibly reached.
            - dict_distances (dict): Dictionary mapping each node to a tuple of (depth, total_distance),
                                    where depth is the number of hops from the starting node and
                                    total_distance is the accumulated distance along the path.
    
    Notes:
        - Nodes with underscores (e.g., SID/STAR nodes) are skipped during exploration.
        - The function prioritizes paths with lower depths, and for equal depths, paths with
          shorter distances.
        - When branching_factor is specified, only the top N nodes (according to the heuristic)
          are expanded at each depth layer.
    """
    ...
    while buff:
        ...
        for node in buff:
            ...
            for neighbor in neighbors:
                ...
                if direction == 'forward':
                    edge_distance = G.edges[node, neighbor].get('cost', 1)
                else:
                    edge_distance = G.edges[neighbor, node].get('cost', 1)
...
def evaluate_route(G, route):
    ...
    for i in range(len(route) - 1):
        ...
        if G.has_edge(current_node, next_node):
            ...
            edge_distance = G.edges[current_node, next_node].get('cost', 0)
...
def find_eligible_range_of_nodes_to_sample(route):
    """
    Identifies the longest contiguous range of eligible waypoints in a route for sampling.
    
    This function analyzes a route and finds the longest contiguous sequence of 
    waypoints that are eligible for modification (those that don't contain an 
    underscore, which typically indicates SID/STAR nodes). It only considers
    middle waypoints, excluding the first and last points of the route.
    
    For example: EGLL MIRA_A3 DEW LUNCH BUTTE LEX_A2 UKBB
    The function will return [2, 4] because the eligible waypoints are DEW LUNCH BUTTE
    
    Parameters:
        route (list): A list of waypoint IDs representing the route.
        
    Returns:
        list or None: A list containing two integers [start_index, end_index] representing 
                     the inclusive range of eligible waypoint indices, or None if no 
                     eligible range exists (e.g., route is too short or has no eligible 
                     middle waypoints).
    
    Notes:
        - Waypoints containing underscores are considered ineligible for sampling.
        - The function only considers middle waypoints (indices 1 to len(route)-2).
        - Returns the longest contiguous block of eligible indices if multiple exist.
    """
...
def mcmc_step(route_graph, route, temperature = 1000, max_depth = 4,
              verbose = False, collapse_pivot = True):
    ...
    if verbose:
        print(f'Picking the modification window\'s first pointer a = {a} from eligible range {eligible_range}')
    # Sample index c, the route at index c (inclusive) onward until the end of the route will probably be replaced
    # i.e., modification window is [a, c)
    ...
    if c_max < a+1:
        ...
        if a+1 == c_max:
            c = a+1 # c = a+1 = c_max, there is just one index value to choose from
        else:
            c = np.random.randint(a+1, c_max)
        if verbose:
            print(f'a = {a}, c = {c}. Modify: {route[a-2]} {route[a-1]} [{" ".join(route[a:c])}] {route[c]} {" " + route[c+1] if c + 1 < len(route) else ""}')
    ...
    if verbose:
        ...
        print(f'route[a-1] = {route[a-1]}, route[c] = {route[c]}. Finding forward admissible nodes from {route[a-1]}.')
    ...
    if collapse_pivot:
        ...
        # When not collapsing, use the original distances and create a mapping from each node to itself
        V_distances_collapsed = V_distances
        node_to_collapsed_node_mapping = {node: node for node in V_distances}
    ...
    if verbose:
        ...
        print(f'New route: {new_route[new_a-2]} {new_route[new_a-1]} [{" ".join(new_route[new_a:new_c])}] {new_route[new_c]} {" " + new_route[new_c+1] if new_c + 1 < len(new_route) else ""}')
