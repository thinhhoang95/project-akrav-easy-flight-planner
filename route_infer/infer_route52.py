...
def process_flight_segments(graph, segments_df, error_threshold=7.5, max_radius=12, min_radius=2,
    ...
    Parameters:
        ...
        segments_df: DataFrame containing flight segments with columns:
                     from_lat, from_lon, to_lat, to_lon, from_time, to_time, from_speed, to_speed, from_alt, to_alt
    ...
    return full_route, new_nodes, pass_times, speeds, alts

def consolidate_nodes(route, nodes, pass_times, speeds, alts):
    """
    ...
    Parameters:
        ...
        alts: List of tuples (alt_A, alt_B)
    Returns:
        tuple: (new_route, new_nodes, new_pass_times, new_speeds, new_alts) where:
               ...
               - new_alts is the list of altitudes
    """
    ...
    return new_route, new_nodes, new_pass_times, new_speeds, new_alts
...
def extract_waypoints_from_augmented_route(augmented_route_object, skip_synthetic_waypoints=False):
    ...
    speeds = []
    alts = []
    # Process each segment to extract waypoints
    for i, segment in enumerate(augmented_route_object):
        from_node, to_node, _, pass_time_from, pass_time_to, speed_from, speed_to, alt_from, alt_to = segment
        
        # Add from_node if not a synthetic waypoint to be skipped
        if (not skip_synthetic_waypoints or not from_node.startswith('_')):
            # Check if this waypoint is already in our list to avoid duplicates
            if from_node not in waypoints:
                waypoints.append(from_node)
                pass_times.append(str(pass_time_from))
                speeds.append(str(speed_from))
                alts.append(str(alt_from))
        # Add to_node for the last segment if not a synthetic waypoint to be skipped
        if i == len(augmented_route_object) - 1 and (not skip_synthetic_waypoints or not to_node.startswith('_')):
            # Check if this waypoint is already in our list to avoid duplicates
            if to_node not in waypoints:
                waypoints.append(to_node)
                pass_times.append(str(pass_time_to))
                speeds.append(str(speed_to))
                alts.append(str(alt_to))
    # Convert lists to space-separated strings
    route_str = ' '.join(waypoints)
    pass_times_str = ' '.join([str(round(float(pt))) for pt in pass_times])
    speeds_str = ' '.join([f'{float(spd):.4f}' for spd in speeds])
    alts_str = ' '.join([str(round(float(alt))) for alt in alts])
    return route_str, pass_times_str, speeds_str, alts_str
...
def find_route(graph, segments_df, error_threshold=7.5, distance_threshold_for_segment_skipping=20, max_wp_search_radius=7, min_wp_search_radius=2,
    ...
    # import time
    # start_time = time.time()
    final_route, new_nodes, pass_times, speeds, alts = process_flight_segments(graph, segments_df, error_threshold=error_threshold, max_radius=max_wp_search_radius,
                                                     min_radius=min_wp_search_radius, spatial_index=spatial_index, cell_size=cell_size)
    final_route, new_nodes, pass_times, speeds, alts = consolidate_nodes(final_route, new_nodes, pass_times, speeds, alts)
    
    augmented_route_object = [] # (node_A, node_B, distance, pass_time_A, pass_time_B, speed_A, speed_B, alt_A, alt_B)
    for i in range(len(final_route)):
        ...
        alt_A, alt_B = alts[i]
        augmented_route_object.append((node_A, node_B, distance, pass_time_A, pass_time_B, speed_A, speed_B, alt_A, alt_B))
